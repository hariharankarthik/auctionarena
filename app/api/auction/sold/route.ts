...
export async function POST(req: NextRequest) {
  ...
  // Timer-driven auctions: prefer /api/auction/finalize.
  ...
  if (!room.current_bidder_team_id) {
    return NextResponse.json(
      {
        error:
          "No winning bid yet — the hammer only completes a sale to the current high bidder. Wait for a bid, or use Unsold to pass this player.",
      },
      { status: 400 },
    );
  }
  ...
  return NextResponse.json({ success: true, completed, was_sold: true });
}
