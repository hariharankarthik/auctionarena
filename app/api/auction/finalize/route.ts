...
export async function POST(req: NextRequest) {
  ...
  const body = (await req.json()) as { room_id?: string; force?: boolean };
  ...
  if (room.status !== "live") return NextResponse.json({ error: "Auction not live" }, { status: 400 });
  if (!room.current_player_id) return NextResponse.json({ error: "No active player" }, { status: 400 });

  const now = Date.now();
  const ends = room.lot_ends_at ? new Date(room.lot_ends_at).getTime() : null;
  if (!force && ends && now < ends) {
    return NextResponse.json({ error: "Lot timer not expired" }, { status: 400 });
  }
